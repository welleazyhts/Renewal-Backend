...
class BotCallingProviderConfigAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ('Basic Information', {
            'fields': ('name', 'provider_type', 'priority', 'is_default', 'is_active', 'bot_script')
        }),
    ...
    def activate_providers(self, request, queryset):
        ...
        self.message_user(request, f"{count} bot calling providers activated successfully.")

    activate_providers.short_description = "Activate selected bot calling providers"

    def deactivate_providers(self, request, queryset):
        count = queryset.update(is_active=False)
        self.message_user(request, f"{count} bot calling providers deactivated successfully.")

    deactivate_providers.short_description = "Deactivate selected bot calling providers"

    def reset_daily_usage(self, request, queryset):
        for provider in queryset:
            provider.reset_daily_usage()
        self.message_user(request, f"Daily usage reset for {queryset.count()} bot calling providers.")
    ...
    def reset_monthly_usage(self, request, queryset):
        ...
        self.message_user(request, f"Monthly usage reset for {queryset.count()} bot calling providers.")
